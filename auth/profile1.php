...
<main class="max-w-7xl mx-auto px-4 py-8">
  ...
  <form method="post" enctype="multipart/form-data" class="bg-white shadow rounded-2xl p-6 grid gap-4">
    <div>
      ...
      <input type="text" name="full_name" value="<?= htmlspecialchars($user['full_name'] ?? '') ?>" class="border rounded-xl p-2 w-average">
    </div>
    <div>
      <label class="block text-sm font-medium">Email</label>
      <input type="email" name="email" value="<?= htmlspecialchars($user['email']) ?>" class="border rounded-xl p-2 w-average" required>
    </div>
    <div>
      <label class="block text-sm font-medium">Téléphone</label>
      <input type="text" name="phone" value="<?= htmlspecialchars($user['phone'] ?? '') ?>" class="border rounded-xl p-2 w-average">
    </div>
    <div>
      <label class="block text-sm font-medium">Adresse</label>
      <input type="text" name="address" value="<?= htmlspecialchars($user['address'] ?? '') ?>" class="border rounded-xl p-2 w-average">
    </div>
    <div>
      <label class="block text-sm font-medium">Bio</label>
      <textarea name="bio" class="border rounded-xl p-2 w-average h-24"><?= htmlspecialchars($user['bio'] ?? '') ?></textarea>
    </div>
    <div>
      ...
      <?php endif; ?>
      <input type="file" name="photo" class="border rounded-xl p-2 w-average">
    </div>
